...
class DenseKNNNeighbourFinder(BaseNeighbourFinder):
    """"k-Nearest Neighbor for Dense format.
    ...
    def __call__(self, x, y):
        ...
        Args:
            x (torch.tensor): Features of all points. (B,N,C)
            y (torch.tesnor): Features of center points. (B,M,C)

        Return:
            indexes (torch.tensor): K neighbour indexes. (B, M, k)
        """
    ...
    @staticmethod
    def dense_to_sparse(dense_inputs):
        B, N, C = dense_inputs.shape
        sparse_inputs = dense_inputs.view(B*N, C)
        batch = torch.arange(0, B, device=dense_inputs.device).view(B, 1) \
            .repeat(1, N).view(B*N)
        return sparse_inputs, batch
